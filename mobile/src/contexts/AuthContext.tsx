  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      if (Platform.OS === 'web' && typeof window !== 'undefined') {
        ...
        if (hash && hash.includes('refresh_token=')) {
          const params = new URLSearchParams(hash.substring(1));
          const refresh_token = params.get('refresh_token');
          if (refresh_token) {
            // Use refreshSession instead of setSession to avoid 401 on /auth/v1/user
            ...
            if (error) console.warn('OAuth refresh failed:', error.message);
          }
        }
      }
